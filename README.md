# CSCE-4901-Team-Red
CSCE 4901 Capstone Project - Team Red

## Team Name
Team Red

## Team Members
- Anusha Shah
- Srijal Prajapati
- Colin Baehr
- Alffy Joseph
- Thelonious Brown

## Project Description
This repository contains the source code, documentation, and project materials for Team Red's CSCE 4901 Capstone Project.

## Current Sprint
Sprint 0 - Requirements Gathering and Project Planning

## Sprint 0 Goals
- Understand the project domain
- Gather and document overall project requirements
- Identify project goals and features
- Organize tasks using our Kanban board
- Prepare the project for future development sprints
